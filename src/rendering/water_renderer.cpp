...
void WaterRenderer::render(VkCommandBuffer cmd, VkDescriptorSet perFrameSet,
    ...
    for (const auto& surface : surfaces) {
        ...
        const float waveAmp =
            classicWater_ ? 0.0f
                          : (isWmoWater ? 0.0f
                                        : (basicType == 1 ? 0.35f : 0.08f));
        const float waveFreq = classicWater_
            ? 0.0f
            : (canalProfile ? 0.35f : (basicType == 1 ? 0.20f : 0.30f));
        const float waveSpeed = classicWater_
            ? 0.0f
            : (canalProfile ? 1.00f : (basicType == 1 ? 1.20f : 1.40f));
